...
public class EventsDAO {
    ...
    public boolean AddEvent(EventsDTO event) throws SQLException{
        ...
        try{
            ...
            if (conn!=null){
                String sql= "INSERT INTO tblEvents(UserID,EventContent, EventDescription,Followers,MaxFollower,Status)"
                        + " VALUES (?,?,?,?,?,0)";
                ...
                stm.setInt(4,0);
                stm.setInt(5, event.getMaxFollowers());
    ...
    public List<EventsDTO> ListEventByUserID(int userID) throws NamingException, SQLException{
        ...
        try{
            conn=DButils.getConnection1();
            if (conn!=null){
                String sql="SELECT u.Username,e.EventID,e.UserID,e.EventContent,e.Followers,e.MaxFollower,t.starttime,t.endtime,l.LocationName,e.EventDescription,e.IsBanned,e.Status \n" +
                            "FROM tblEvents e,tblTimetable t,tblLocation l,tblUsers u\n" +
                            "WHERE e.UserID = ? AND e.EventID=t.EventID AND l.LocationID=t.LocationID and e.userID=u.userID\n" +
                ...
                while (rs.next()){
                    ...
                    ,rs.getInt("Followers"),rs.getInt("MaxFollower"),rs.getString("EventDescription"),rs.getBoolean("Status"),rs.getBoolean("IsBanned"));
                    list.add(eva);
                }   
            }
        }finally{
    ...
    public List<EventsDTO> SearchEvents(String eventContent) throws NamingException, SQLException{
        ...
        try{
            conn=DButils.getConnection1();
            if (conn!=null){
                String sql="SELECT u.Username,e.EventID,e.UserID,e.EventContent,e.Followers,e.MaxFollower,t.starttime,t.endtime,l.LocationName,e.EventDescription  \n" +
                        "FROM tblEvents e,tblTimetable t,tblLocation l,tblUsers u\n" +
                ...
                while (rs.next()){
                   EventsDTO eva=new EventsDTO(rs.getInt("EventID"),rs.getInt("UserID"),rs.getString("LocationName"),
                    rs.getString("UserName"),rs.getString("starttime"),rs.getString("endtime"),rs.getString("EventContent")
                    ,rs.getInt("Followers"),rs.getInt("MaxFollower"),rs.getString("EventDescription"));
                    list.add(eva);
                }   
            }
        }finally{
...
"SET @currentdate = ?\n" +
"SELECT u.Username,e.EventID,e.UserID,e.EventContent,e.Followers,e.MaxFollower,t.starttime,t.endtime,l.LocationName,e.EventDescription  \n" +
"FROM tblEvents e ,tblTimetable t,tblLocation l,tblUsers u\n" +
    ...
    public List<EventsDTO> ViewPendingEvents() throws NamingException, SQLException{
        ...
        try{
            ...
            if (conn!=null){
                String sql="SELECT u.Username,e.EventID,e.UserID,e.EventContent,e.Followers,e.MaxFollower,t.starttime,t.endtime,l.LocationName,e.EventDescription \n" +
"FROM tblEvents e,tblTimetable t,tblLocation l,tblUsers u \n" +
"Where e.status=0 AND e.EventID=t.EventID AND l.LocationID=t.LocationID and e.userID=u.userID AND e.IsBanned is null \n" +
"ORDER BY e.EventID DESC";
                stm=conn.prepareStatement(sql);
                rs=stm.executeQuery();
                String a="a";
    ...
    public EventsDTO GetEventByID(int eventID) throws SQLException{
        ...
        try{
            conn=DButils.getConnection1();
            if (conn!=null){
                String sql="SELECT u.FullName,e.EventID,e.UserID,e.EventContent,e.Followers,e.MaxFollower,t.starttime,t.endtime,l.LocationName,e.EventDescription \n" +
"FROM tblEvents e,tblTimetable t,tblLocation l,tblUsers u\n" +
"where e.EventID = ? AND e.EventID=t.EventID AND l.LocationID=t.LocationID and e.userID=u.userID";
                stm=conn.prepareStatement(sql);
                stm.setInt(1, eventID);
                rs=stm.executeQuery();
                if(rs.next()) {
                    EventsDTO eva=new EventsDTO(rs.getInt("EventID"),rs.getInt("UserID"),rs.getString("LocationName"),
                    rs.getString("FullName"),rs.getString("starttime"),rs.getString("endtime"),rs.getString("EventContent")
                    ,rs.getInt("Followers"),rs.getInt("MaxFollower"),rs.getString("EventDescription"));
                    res= eva;
                }
            }
    ...
    public boolean UpdateBanned(int eventID) throws SQLException{
        boolean check=false;
        try{
            conn=DButils.getConnection1();
            if (conn!=null){
                String sql="UPDATE tblEvents SET IsBanned=1 WHERE eventID=?";
                stm=conn.prepareStatement(sql);
                stm.setInt(1, eventID);
                int re=stm.executeUpdate();
                check= re>0 ? true:false;
