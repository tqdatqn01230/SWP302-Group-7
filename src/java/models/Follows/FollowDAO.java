...
public class FollowDAO implements Serializable{
    ...
    public boolean AddFollow(FollowDTO follow) throws NamingException, SQLException, ClassNotFoundException{
        ...
        try{
            ...
            if (conn!=null){
                ...
                int a= follow.getEventID();int b=follow.getUserID();
    ...
    public boolean ChangeNotificationWhenReply(String noti,int eventID,int userID) throws SQLException, NamingException, ClassNotFoundException{
        boolean check=false;
        try{
            conn =DButils.getConnection1();
            if (conn!=null){
                String sql="UPDATE tblFollow "
                        + " SET Notifications= ?,    Status=0 Where eventID = ? AND UserID=?";
                stm= conn.prepareStatement(sql);
                stm.setString(1, noti);
                stm.setInt(2, eventID);
                stm.setInt(3, userID);
    ...
    public boolean SeeNotification(int userID,int eventID) throws SQLException, NamingException, ClassNotFoundException{
        boolean check=false;
        try{
            conn =DButils.getConnection1();
            if (conn!=null){
                String sql="UPDATE tblFollow "
                        + " SET Status=1  Where userID = ? AND eventID=? ";
                stm= conn.prepareStatement(sql);
                stm.setInt(1, userID);
                stm.setInt(2, eventID);
                int re=stm.executeUpdate();
    ...
    public FollowDTO GetFollow(int userID,int eventID) throws SQLException{
        FollowDTO list= null;
        try{
            conn=DButils.getConnection1();
            if (conn!=null){
                String sql="SELECT * FROM tblFollow WHERE userID=? AND EventID=?";
                stm=conn.prepareStatement(sql);
                stm.setInt(1, userID);
                stm.setInt(2, eventID);
                rs=stm.executeQuery();
                while (rs.next()){
                    list=new FollowDTO(rs.getInt("eventID"), 
                            rs.getInt("userID"), rs.getString("Notifications"));
                }
            }
        }finally{
