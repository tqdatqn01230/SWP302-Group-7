...
public class LocationDAO {
    ...
    public LocationDTO getLocationbyID(int ID) throws SQLException{
        ...
        try{
            conn=DButils.getConnection1();
            if (conn!=null){
                String sql="SELECT * FROM tblLocation Where LocationID=?";
                stm=conn.prepareStatement(sql);
                stm.setInt(1, ID);
                rs=stm.executeQuery();
                if (rs.next()) res=new LocationDTO(rs.getInt("LocationID"), rs.getString("LocationName"));
    ...
    public List<LocationDTO> getListLocation() throws SQLException{
        ...
        try{
            conn=DButils.getConnection1();
            if (conn!=null){
                String sql="SELECT LocationName,LocationID FROM tblLocation Where Status=1";
                stm=conn.prepareStatement(sql);
                rs=stm.executeQuery();
                while (rs.next()) {
                    String a=rs.getString("LocationName");
                    list.add(new LocationDTO(rs.getInt("LocationID"),a));
                }
            }
        }finally{
            CloseConnection();
